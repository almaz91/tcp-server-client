...
int main()
{
  //Создание объекта TcpServer с передачей аргументами порта и лябда-фунции для обработки клиента
  TcpServer server( 40000,
      [&](TcpServer::Client client)
        {
             ...
             if (!server.saveData(client.getData(), size))
             {
                 {
                   // Если на сервере нет места для записи пришедшего от клиента файла
                     std::cout << "No memory for share on the server!" << std::endl;
                 }
             }
        }
