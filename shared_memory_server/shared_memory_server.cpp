...
// Задаёт callback-функцию запускаемую при подключении клиента
...
bool TcpServer::saveData(char* data, int data_size)
{
    //std::lock_guard<std::mutex> g(m_lock);
    if ( shared_memory_size - idx < data_size)
    {
        //_status = status::err_shared_memory;
        return false;
    }
    int i = 0;
    for(; i < data_size; i++)
    {
        GUID.at(idx + i) = data[i];
    }
    idx = i;
    return true;
}
...
void TcpServer::stop() {
    ...
    shutdown(serv_socket, 0);    // Обрыв соединения сокета
    close(serv_socket);
    joinLoop();
